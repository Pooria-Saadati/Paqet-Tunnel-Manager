# Paqet memory/resource leak investigation

## Observed failure mode

On a long-running client with an unstable remote path, repeated reconnects were followed by monotonic growth in process resources:

- PACKET sockets
- ~8 MiB mappings associated with those packet sockets
- file descriptors
- threads blocked in polling
- RSS/PSS
...
On a small VPS with no swap, the leak eventually pushed the host into memory pressure. Historical `sar` and journal data showed:
...
This creates a feedback loop: resource growth increases host latency, which causes more tunnel health-check failures, which can cause more reconnect activity.
...
For this fork, the current custom `v2.2.0-optimized-Behzad` binary must not be treated as a trusted default until it has source provenance and the leak is proven fixed. The preferred migration target for testing is the official upstream `v1.0.0-alpha.21` release, with both ends upgraded together when protocol compatibility requires it.

## Resource guard

`scripts/paqet-resource-guard.sh` is a mitigation and diagnostics tool. It is not a core leak fix.
...
A candidate core passes this regression test when reconnect count can rise while the following stay bounded over time:
